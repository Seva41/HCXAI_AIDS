...
import Modulos.Analyzer.Ataque as a
import Modulos.Analyzer.DataAtaque as dt
import configparser #https://docs.python.org/3/library/configparser.html

config = configparser.ConfigParser() #para obtener la info desde config.ini
config.read("config.ini")

RUTA_REGISTRO_ATAQUES = config['ANALYZER']['RUTA_REGISTRO_ATAQUE']
RUTA_ATAQUES_PROCESADOS = config['ANALYZER']['RUTA_ATAQUES_PROCESADOS']
RUTA_LOG_MONITORING = config['SERVER']['RUTA_DATA_INFO']
...
'''
TOLERANCIA_ATAQUES = {
        "Analysis": int(config['TOLERANCIA_ATAQUES']['Analysis']),
        "Backdoor": int(config['TOLERANCIA_ATAQUES']['Backdoor']),
        'DoS':      int(config['TOLERANCIA_ATAQUES']['DoS']),
        'Exploits': int(config['TOLERANCIA_ATAQUES']['Exploits']),
        "Fuzzers":  int(config['TOLERANCIA_ATAQUES']['Fuzzers']),
        "Generic":  int(config['TOLERANCIA_ATAQUES']['Generic']),
        "Reconnaissance": int(config['TOLERANCIA_ATAQUES']['Reconnaissance']),
        "Shellcode": int(config['TOLERANCIA_ATAQUES']['Shellcode']),
        "Worms":    int(config['TOLERANCIA_ATAQUES']['Worms'])
    }
...
'''
TOLERANCIA_TIEMPO = {
        "Analysis":(int(config['TOLERANCIA_TIEMPO']['Analysis_cantidad']), config['TOLERANCIA_TIEMPO']['Analysis_TIME']),
        "Backdoor":(int(config['TOLERANCIA_TIEMPO']['Backdoor_cantidad']), config['TOLERANCIA_TIEMPO']['Backdoor_TIME']),
        'DoS': (int(config['TOLERANCIA_TIEMPO']['DoS_cantidad']), config['TOLERANCIA_TIEMPO']['DoS_TIME']),
        'Exploits':(int(config['TOLERANCIA_TIEMPO']['Exploits_cantidad']), config['TOLERANCIA_TIEMPO']['Exploits_TIME']),
        "Fuzzers": (int(config['TOLERANCIA_TIEMPO']['Fuzzers_cantidad']), config['TOLERANCIA_TIEMPO']['Fuzzers_TIME']),
        "Generic": (int(config['TOLERANCIA_TIEMPO']['Generic_cantidad']), config['TOLERANCIA_TIEMPO']['Generic_TIME']),
        "Reconnaissance":(int(config['TOLERANCIA_TIEMPO']['Reconnaissance_cantidad']), config['TOLERANCIA_TIEMPO']['Reconnaissance_TIME']),
        "Shellcode": (int(config['TOLERANCIA_TIEMPO']['Shellcode_cantidad']), config['TOLERANCIA_TIEMPO']['Shellcode_TIME']),
        "Worms": (int(config['TOLERANCIA_TIEMPO']['Worms_cantidad']), config['TOLERANCIA_TIEMPO']['Worms_TIME'])
    }
...
class Analyzer:
    ...
    def processData(self):
        ...
        for item in listaDataAtaque: #cada item es DataAtaque
            ...
            #5) ip source
            puerto, cantidadAtaques, fechas, horas = item.getData() 

            for key, value in fechas.items():
                if int(cantidadAtaques[key]) > self.__toleranciaAtaques[key]:
                    if self.analizarVentana(value[0], value[1], key, int(cantidadAtaques[key]) > self.__toleranciaAtaques[key]):
    ...
    '''
    def analizarVentana(self, fechaPrimera, fechaUltima, tipoAtaque, cantidadAtaques):
