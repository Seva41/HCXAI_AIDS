...
class Executer:
        ...
        if error != "port error":
            if flag != "error":
                if not self.comprobarAcciones(str(port), name):
                    ...
                    print(
                        "Saltando: port ->{} func ->{} (ya aplicado)".format(port, name)
                    )
            else:
                return "flag error"
        else:
    ...
    def log(self, name, port, sintoma):
        ...
        try:
            ...
            logger.addHandler(handler)
            logger.info(
                "Funcion: {} \t puerto/ip: {} \t Fecha: {} \t Hora: {} \t Sintoma: {}".format(
                    name, port, fecha, hora, sintoma
                )
            )
    ...
    def report(self, comandosExec):
        ...
        port_type = (
            []
        )  # matriz de la siguiente forma [ [puerto, [medidas]] ] *medidas es otra lista; son strings todos los contenidos
        ...
        for item in port_type:
            ...
            try:
                port = (
                    item[0] if item[0] != "https" else 443
                )  # si el puerto no es "https" lo deja tal cual, sino lo iguala a 443
        ...
        for key, value in MEDIDAS_TOMADAS.items():
            if key == "BlockIP":
                print(
                    "La acción: {} se aplicará a las siguientes IP(s):\t {}".format(
                        key, value
                    )
                )
            else:
                print(
                    "La acción: {} se aplicará a los siguientes puerto(s):\t {}".format(
                        key, value
                    )
                )
